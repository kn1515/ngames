...
func drawBoard(display st7789.Device) {
	// 原点を90度回転して描画するため、出力画像のサイズは入れ替え
	w := GRID_W * BLOCK_SIZE
	h := GRID_H * BLOCK_SIZE
	rw := h // rotated width
	rh := w // rotated height
	img := pixel.NewImage[pixel.RGB565BE](rw, rh)
	bg := pixel.NewColor[pixel.RGB565BE](0,0,20)
	// 背景を一度のループで埋める
	for i:=0;i<rw*rh;i++{ x := i % rw; y := i / rw; img.Set(x,y,bg) }

	// ボードセルを順次描画（各セルを回転位置にマッピング）
	for i:=0;i<GRID_W*GRID_H;i++{
		v := board[i]
		if v==0 { continue }
		gx := i % GRID_W
		gy := i / GRID_W
		// 回転（時計回り90度）: (gx,gy) -> (newGX, newGY)
		newGX := GRID_H - 1 - gy
		newGY := gx
		x := newGX * BLOCK_SIZE
		y := newGY * BLOCK_SIZE
	...
	// 回転画像を画面中央に描画
	display.DrawBitmap(int16((240-rw)/2), int16((240-rh)/2), img)
}
